...
After cloning the repository, create, activate, and install the necessary packages in a virtual environment for Python 3 using the `requirements.txt` file:
```
python -m venv my_env
source my_env/bin/activate
python3 -m pip install -r requirements.txt
```
Note that a different syntax may be required for Windows; the commands listed here will work for Unix/macOS. If an error occurs please make sure virtualenv is installed (e.g., `pip install virtualenv`).
...
3. Once the data is ready, to train all the models using the hyperparameters specified in the final project report, run:
   ```
   bash run.sh
   ```
   This will run experiments for all data (MFCC, GFCC) and task (emotion classification, intensity classification) settings.

   After each model is trained, a set of scikit-learn classification reports and confusion matrices will be generated for the model to assess its performance on the train, validation, and test splits of the data. These results will be saved in a newly and automatically created `./results` directory.
4. Once all models have completed training, aggregate the scores of performance for each data and task setting by running:
   ```
   python analyze.py
   ```
